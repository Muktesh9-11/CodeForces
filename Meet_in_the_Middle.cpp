...
void solve(ll x, ll l, ll r, vector<ll>arr, ll sum, unordered_map<ll,ll>&mpp){
    //code here
    ...
    solve(x,l+1,r,arr,sum,mpp);

    if(sum <= x - arr[l]) solve(x,l+1,r,arr,sum+arr[l],mpp);
...
int main(){
   ...
   // vector<vector<ll> >dp(n/2,vector<ll>(total+1,-1));
    unordered_map<ll,ll> freq1;
    freq1.reserve(1<<20);
    solve(x,0,n/2-1,arr,0,freq1);
    //vector<ll> sum1 = storeSum;

    //storeSum.clear();
   // vector<vector<ll>>dp2(n/2,vector<ll>(total+1,-1));


    unordered_map<ll,ll> freq2;
    freq2.reserve(1<<20);
    solve(x,n/2,n-1,arr,0,freq2);
    //vector<ll> sum2 = storeSum;

    ll cnt = 0;
    for(auto it : freq1){
        int f2 = x - it.first;
        if(freq2.find(f2) != freq2.end()){
            cnt += 1ll* it.second * freq2[f2];
        }
    }
