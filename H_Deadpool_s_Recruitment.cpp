...
bool customS(const pair<double, int> &a, const pair<double, int> &b) {
    if (a.first == b.first)
        return a.second > b.second;  // Descending order for second element
...
int main(){
        ...
        for(int i=0;i<n;i++){
            double a1,a2;
            a1 = (vec[i].first*t1)+(vec[i].second*t2);
            a1 -= (vec[i].first*t1)*k/100;

            a2 = (vec[i].second*t1)+(vec[i].first*t2);
            a2 -= (vec[i].second*t1)*k/100;

            double x = max(a1,a2);
            ans.push_back(make_pair(x,i+1));
        }

        sort(ans.begin(),ans.end(),customS);
